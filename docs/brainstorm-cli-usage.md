...
```bash
python brainstorm_from_kb.py ask --query "哪些文章讨论行业轮动和ETF配置？"
```

## 脑暴模式

```bash
python brainstorm_from_kb.py brainstorm --query "基于行业轮动和风险预算，给我5个新的ETF配置想法"
```
...
```bash
python brainstorm_from_kb.py brainstorm --query "行业轮动和风险预算" --retrieval hybrid
python brainstorm_from_kb.py brainstorm --query "行业轮动和风险预算" --retrieval vector
python brainstorm_from_kb.py brainstorm --query "行业轮动和风险预算" --retrieval keyword
```
...
```bash
python brainstorm_from_kb.py brainstorm ^
...
```bash
python brainstorm_from_kb.py brainstorm --query "行业轮动和风险预算" --retrieval hybrid --dry-run
```
