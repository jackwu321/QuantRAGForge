...
**Goal:** Add a Karpathy-style compiled wiki layer (`wiki/concepts/` + `wiki/sources/`) plus CLI ingest extension to web URLs and PDFs; wire 4 new agent tools and rebuild brainstorm to retrieve concept articles first.

**Architecture:** Per-article enrichment unchanged. New `compile_wiki` step writes mechanical source summaries plus LLM-synthesized concept articles with status lifecycle (proposed → stable → deprecated). Wiki entries embed into the same ChromaDB collection with a `kb_layer` metadata field. Brainstorm retrieves concept articles first, complementary raw articles second.
...
| `agent/tools.py` | 4 new tools, `embed_knowledge`/`ingest_article` updated |
| `agent/prompts.py` | Add wiki-layer guidance |
| `README.md` | Reflect 12-tool surface, new pipeline step, new ingest sources |
...
Note on `sources:` parsing: the existing `parse_frontmatter` in `kb_shared` only handles single-line `key: value` pairs, so the YAML block-list form (`sources:\n  - path1\n  - path2`) won't round-trip through frontmatter. Workaround: the *source of truth* for sources is the body's `## Sources` section (parsed as `[[basename]]` wikilinks); the frontmatter `sources:` field is treated as an inline-list optimization. `parse_concept` reconstructs the `sources` list from `## Sources` wikilinks via `source_basenames`, then uses caller-supplied `articles/.../article.md` paths during compile. Block-list form in `serialize_concept` is emitted for human readability but parsed back from the `## Sources` section.
...
def _newer(article_dir: Path, concept: ConceptArticle | None) -> bool:
    ...
    try:
        compiled_ts = date.fromisoformat(concept.last_compiled).toordinal()
    except ValueError:
        return True
    # Convert mtime (seconds) to ordinal day
    from datetime import datetime as _dt
    art_day = _dt.fromtimestamp(art_mtime).date().toordinal()
    return art_day > compiled_ts
...
def compile_wiki(mode: str = "incremental", dry_run: bool = False) -> str:
    ...
    if report.concepts_proposed:
        summary += f"\n\nProposed concepts ({report.concepts_proposed}) are awaiting review. Use list_concepts to see them and set_concept_status to approve/reject."
    return summary
```
...
### Task D3: `set_concept_status` agent tool
...
git commit -m "Register 4 new wiki tools and update agent system prompt"
```
...
def _retrieve_concept_articles(query: str, top_k: int = DEFAULT_CONCEPT_TOP_K) -> list[dict]:
    """Retrieve top-K wiki/concepts/*.md by simple query-vs-title-and-aliases match.

    Returns a list of dicts: {slug, title, sources, body_text}.
    """
    cdir = WIKI_DIR / "concepts"
    if not cdir.exists():
        return []
...
```
  [5.5 Compile Wiki] ──> wiki/concepts/<concept>.md + wiki/sources/<article>.md + wiki/INDEX.md
        |               (incremental: per-article LLM call + per-affected-concept LLM call)
        v
...
Replace the existing 8-row table with 12 rows:
...
| `set_concept_status` | Approve, deprecate, or delete a wiki concept |
...
The wiki is a directory of LLM-written, LLM-maintained markdown files synthesized from your articles:
...
**Lifecycle:** `compile_wiki` runs after `sync_articles`. New articles either map to existing concepts (which get recompiled) or trigger an auto-proposed new concept (status: `proposed`, awaits your approval via `set_concept_status`). The seed taxonomy seeds 7 starter concepts (factor-models, factor-timing, regime-detection, momentum-strategies, etf-rotation, risk-parity, volatility-targeting).

**Brainstorm impact:** When you run brainstorm queries, the agent retrieves matching concept articles first (richer synthesis), then complementary raw-article chunks via vector search (excluding sources already cited by the wiki concepts). If the wiki is sparse or missing, brainstorm falls back to the pure-vector path with no behavior change.
```
...
Agent: Synced 2 (1 deleted).
       compile_wiki: 2 sources, 2 concepts recompiled, 1 proposed.
       Proposed concept "macro-momentum" awaits review (use list_concepts then set_concept_status).
       Embedded 2 articles + 8 wiki entries.

You: list proposed concepts
Agent: macro-momentum — Macro Momentum (proposed) — 2 source(s)

You: approve macro-momentum
Agent: Concept macro-momentum → stable.
...
```markdown
- **Compiled wiki layer** — LLM synthesizes raw articles into concept articles with backlinks; brainstorm queries pull from these synthesized concepts first
...
```bash
git add README.md
git commit -m "Update README for wiki layer, 12-tool surface, multi-source ingest"
```
...
Run this checklist after the implementation is complete and tests pass. Each item produces a real artifact you can inspect.
...
- [ ] Verify: `wiki/INDEX.md` lists concepts grouped by content_type
- [ ] Verify: any auto-proposed concepts appear under "## Proposed Concepts"
...
- [ ] Run agent flow: enrich → review → set high_value → sync → compile_wiki
...
## 6. Auto-proposed concept lifecycle
- [ ] Ingest an article on a topic outside the seed taxonomy (e.g. "options volatility surface")
- [ ] Run compile_wiki
- [ ] Verify a `proposed` concept appears in `list_concepts(status='proposed')`
- [ ] Use the agent: `set_concept_status` to approve or reject
- [ ] If approved, run compile_wiki again — verify the concept gets recompiled with sources
...
Run: `python3 agent_cli.py --query "list concepts"`
Expected: agent invokes `list_concepts` tool and returns the 7 seed concepts (or an empty/initialized message if wiki not yet bootstrapped).
